 ...
 * 2x1 时间结余小组件
 */
class WidgetProvider2x1 : AppWidgetProvider() {

    companion object {
        const val ACTION_CLICK = "com.example.aiaccounting.widget.ACTION_CLICK_2x1"

        fun updateAllWidgets(context: Context) {
            val appWidgetManager = AppWidgetManager.getInstance(context)
            val componentName = ComponentName(context, WidgetProvider2x1::class.java)
        ...
        fun updateAppWidget(context: Context, appWidgetManager: AppWidgetManager, appWidgetId: Int) {
            val views = RemoteViews(context.packageName, R.layout.widget_accounting_2x1)
